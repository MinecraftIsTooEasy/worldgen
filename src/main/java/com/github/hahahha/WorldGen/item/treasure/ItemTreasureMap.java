...
public class ItemTreasureMap extends Item {
    ...
    public boolean onItemRightClick(EntityPlayer player, float partial_tick, boolean ctrl_is_down) {
        ...
        if (world == null) {
            send(player, t(
                    "worldgen.item.treasure_map.world_unavailable",
        ...
        send(
                player,
                tf(
                        "worldgen.item.treasure_map.bind.success",
        ...
        send(
                player,
                t(
                        "worldgen.item.treasure_map.bind.locked",
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List info, boolean extended_info, Slot slot) {
        ...
        if (!isBound(tag)) {
            info.add(tf(
                    "worldgen.item.treasure_map.tooltip.target",
                    "Target: %s",
                    this.definition.preferredTargetLabel()));
            info.add(t(
                    "worldgen.item.treasure_map.tooltip.unbound",
        ...
        info.add(tf("worldgen.item.treasure_map.tooltip.bound", "Bound: %s", label));
        info.add(tf("worldgen.item.treasure_map.tooltip.position", "Pos: (%d, %d, %d)", x, y, z));
        info.add(tf(
                "worldgen.item.treasure_map.tooltip.dimension",
                "Dimension: %s",
        ...
        if (vanillaType != null) {
            if (!vanillaType.supportsDimension(world.getDimensionId())) {
                send(
                        player,
                        tf(
                                "worldgen.item.treasure_map.search.unsupported_dimension",
    ...
    private void sendNotFound(EntityPlayer player, String targetLabel) {
        send(
                player,
                tf(
                        "worldgen.item.treasure_map.search.not_found",
    ...
    private void sendBoundTarget(EntityPlayer player, NBTTagCompound tag) {
        ...
        send(
                player,
                tf(
                        "worldgen.item.treasure_map.bind.current",
    ...
    private String resolveStoredLabel(NBTTagCompound tag) {
        ...
        return value == null || value.trim().isEmpty() ? this.definition.preferredTargetLabel() : value;
    }
    ...
    private static void send(EntityPlayer player, String text, boolean withPrefix) {
        ...
        String message = withPrefix
                ? tf("worldgen.item.treasure_map.prefix", "[Treasure Map] %s", text)
                : text;
